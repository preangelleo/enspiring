...
def openai_gpt_structured_output(prompt: str, system_prompt: str, chat_id: str, model = ASSISTANT_MAIN_MODEL, engine = engine, user_parameters = {}):
...
def post_journal_to_ghost_creator(prompt: str, chat_id: str, engine = engine, token = TELEGRAM_BOT_TOKEN, model="gpt-4o", message_id = '', user_parameters = {}, is_journal = True):
    if not user_parameters: user_parameters = user_parameters_realtime(chat_id, engine)
...
def repost_journal_to_ghost_creator(chat_id: str, post_id: int, engine = engine, token = TELEGRAM_BOT_TOKEN, model = ASSISTANT_MAIN_MODEL, user_parameters = {}, need_translate = False):
    ...
    post_content_dict = {
        "title": title,
        "slug": slug,
        "tags": [tags] if tags else ['Journal'],
        "html": html_content,
        "status": publish_status,
        "custom_excerpt": custom_excerpt,
        "visibility": visibility,
    ...
    if response.status_code in [200, 201]:
        ...
        callback_update_post_status(chat_id, url_markdown, post_id, token, user_parameters)
