...
def post_words_story_to_ghost(words_list: list, chat_id: str, admin_api_key = BLOG_POST_ADMIN_API_KEY, ghost_url = BLOG_POST_API_URL, engine = engine, token = TELEGRAM_BOT_TOKEN, model=ASSISTANT_DOCUMENT_MODEL, message_id = '', user_parameters = {}):
    ...
    if response.status_code in [200, 201]:
        ...
        callback_social_sharing(chat_id, url_markdown, post_id, table_name, token, '', is_markdown = True)
...
def create_story_and_post_to_ghost(user_prompt: str, chat_id: str, admin_api_key = BLOG_POST_ADMIN_API_KEY, ghost_url = BLOG_POST_API_URL, engine = engine, token = TELEGRAM_BOT_TOKEN, model=ASSISTANT_DOCUMENT_MODEL, message_id = '', user_parameters = {}):
    ...
    if response.status_code in [200, 201]:
        ...
        table_name = 'user_stories_tailored'
        callback_social_sharing(chat_id, url_markdown, post_id, table_name, token, '', is_markdown = True)
...
def update_story_cover_image_to_ghost_webhook(payload: dict, midjourney_images_dir = midjourney_images_dir, admin_api_key = BLOG_POST_ADMIN_API_KEY, ghost_url = BLOG_POST_API_URL, engine = engine, token = os.getenv("TELEGRAM_BOT_TOKEN_ENSPIRING")):
    ...
    if not upscaled_filepath: return send_debug_to_laogege(f"update_story_cover_image_to_ghost_webhook() >> Failed to save the upscaled images for chat_id: {chat_id}")

    if post_type in ['user', 'telegram']:
        captions = [os.path.splitext(os.path.basename(f))[0] for f in upscaled_filepath]
        return send_images_batch(chat_id, upscaled_filepath, captions, token)
            # send_document_from_file(chat_id, png_file_path, 'Image generated by Midjourney.', token)
...
def post_journal_to_ghost_creator_front(prompt, chat_id, engine, token, model = ASSISTANT_MAIN_MODEL, message_id = '', user_parameters = {}, is_journal=True):
    ...
    if not all([admin_api_key, ghost_url]): 
        if is_journal:
            ...
            return post_journal_to_ghost(prompt, chat_id, '',  BLOG_POST_ADMIN_API_KEY, BLOG_POST_API_URL, engine, token, model, message_id, user_parameters)
        else:
            send_message(chat_id, f"Generating the story based on your prompt, please wait about 120 seconds...", token)
            message_id = str(int(message_id) + 1) if message_id else ''
            return create_story_and_post_to_ghost(prompt, chat_id, BLOG_POST_ADMIN_API_KEY, BLOG_POST_API_URL, engine, token, model, message_id, user_parameters)
...
def auto_blog_post(chat_id: str, engine = engine, token = os.getenv("TELEGRAM_BOT_TOKEN_ENSPIRING"), model=ASSISTANT_MAIN_MODEL_BEST, user_parameters = {}):
    ...
    if response.status_code in [200, 201]:
        ...
        if img_url and post_type == 'post': 
            ...
            try: post_to_bluesky(title, custom_excerpt, url, image_path, bluesky_identifier=BLUESKY_IDENTIFIER_CODEXODYSSEY, bluesky_api_key=BLUESKY_API_KEY_CODEXODYSSEY)
