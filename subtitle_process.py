...
def dealing_tg_command(msg: str, chat_id: str, user_parameters, token=TELEGRAM_BOT_TOKEN, engine=engine, message_id=0, command_corrected = False):
    ...
    while i < 2:
        if i == 1:
            if not command_corrected:
                response = openai_gpt_chat(SYSTEM_PROMPT_COMMAND_CORRECTION, msg, chat_id, ASSISTANT_MAIN_MODEL, user_parameters, token)
                if response == 'no': 
                    reply = openai_gpt_chat(f"Try your best to respond the user's prompt", msg, chat_id, ASSISTANT_MAIN_MODEL, user_parameters, token)
