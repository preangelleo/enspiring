...
def post_news_to_ghost_creator_front(user_prompt, chat_id, engine, token, user_parameters):
    ...
    if not all([admin_api_key, ghost_url]): 
        ...
        if chat_id not in white_list_chat_ids and not openai_api_key:
        ...
        webhook_push_table_name('user_news_jobs', chat_id)
        suffix = ''
        if mother_language == 'English': suffix = f"\n\nYour /mother_language is set to default value -- `English`. Meaning you won't have a `Publish in Mother Language` button in your notification once your article is generated. If you want to have a translated version of your generated article, please click /mother_language to set your mother language."
...
FUNCTIONS_TOOLS = [
    {
        ...
        "function": {
            "name": "Generate_News",
            "description": "Generate a News article and post it to the blog platform.",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {"type": "string", "description": "The user's input prompt about the news."},
                },
                "required": ["prompt"]
            }
        }
